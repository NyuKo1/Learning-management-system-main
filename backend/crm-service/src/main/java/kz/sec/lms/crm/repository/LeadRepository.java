...
public interface LeadRepository extends BaseRepository<Lead, Long> {
    ...
    @Query(value = "SELECT DATE_FORMAT(created_at, '%Y-%m') AS mk, COUNT(*) FROM crm_lead WHERE deleted = 0 AND created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH) GROUP BY mk ORDER BY mk ASC", nativeQuery = true)
